...
bake_job_xml=$(curl -s ${artifacts_location}jenkins/blue-green/vmss-blue-green-bake-job.xml${artifacts_location_sas_token})
deploy_job_xml=$(curl -s ${artifacts_location}jenkins/blue-green/vmss-blue-green-deploy-job.xml${artifacts_location_sas_token})
...
run_util_script "jenkins/run-cli-command.sh" -j "$jenkins_url" -ju "$jenkins_username" -jp "$jenkins_password" -c "create-job bake-image" -cif "bake-image-job.xml"

#add bake image job
echo "${deploy_job_xml}" >deploy-job.xml
run_util_script "jenkins/run-cli-command.sh" -j "$jenkins_url" -ju "$jenkins_username" -jp "$jenkins_password" -c "create-job deploy-to-vmss" -cif "deploy-job.xml"
