...
function print_usage() {
  cat <<EOF
https://github.com/Azure/azure-quickstart-templates/tree/master/301-jenkins-vmss-blue-green
Command
  $0
Arguments
...
if [[ -n "$image_name" ]]; then
  run_util_script "jenkins/blue-green/packer-build-tomcat-image.sh" \
    --app_id "$app_id" \
    --app_key "$app_key" \
...
run_util_script "jenkins/run-cli-command.sh" -c "install-plugin azure-vmss -deploy"

run_util_script "jenkins/blue-green/add-vmss-blue-green-jobs.sh" \
